...
def get_scores(gt_pdb, pred):
    ...
    if len(common) >= 3:
        ...
        # fallback if atom layouts match exactly
        same_layout = (
    ...
    # 3) Superimpose (Kabsch): get R, t and apply to full predicted structure
    try:
        R, t = superimpose(ref_for_fit, mob_for_fit)  # returns (3x3, 3,)
    ...
    # 4) RMSD & TM-score on the same correspondence
    try:
        if use_global:
